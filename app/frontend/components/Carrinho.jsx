...
export default function Carrinho({ initialCart }) {
    ...
    const loadCart = async () => {
        const response = await fetch('/carrinho', { headers: { 'Accept': 'application/json' } });
    ...
    const alterarQuantidade = async (id, delta) => {
        let url = delta > 0 ? `/add_to_carrinho/${id}` : `/decrease_quantity/${id}`;
        await fetch(url, { method: 'POST', headers: { 'X-CSRF-Token': getCsrfToken() } });
        loadCart();
    };

    const removerItem = async (id) => {
        await fetch(`/remove_from_carrinho/${id}`, { method: 'DELETE', headers: { 'X-CSRF-Token': getCsrfToken() } });
        loadCart();
    };
        ...
        <>
            <div className="carrinho-container">
                        ...
                        <div key={item.id} className="carrinho-item">
                            ...
                            <div className="carrinho-img">
                                {item.imagem_url ? (
                                    ...
                                    <div style={{ width: "100%", height: "100%", backgroundColor: "#ddd", borderRadius: "8px" }} />
                                )}
                            </div>
                ...
                <div className="carrinho-footer">
                    <h2>Total da compra: <span>R${Number(total).toFixed(2)}</span></h2>
